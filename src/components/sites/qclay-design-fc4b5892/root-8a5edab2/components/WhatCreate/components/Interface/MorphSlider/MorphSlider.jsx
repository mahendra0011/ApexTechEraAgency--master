...
class MorphEngine {
  constructor(container, config) {
    ...
    this.sizes = this.items.map(() => [16, 9])
    this.videoTextures = []
  ...
  loadTextures() {
    this.items.forEach((item, index) => {
      if (item.video) {
        ...
        document.addEventListener('wheel', unlock, { once: true })
        ...
        this.videoTextures.push({ texture, video })
  ...
  prepareNext(dir, from = this.current) {
    const target = this.wrap(from + dir)
  ...
  goTo(dir) {
    ...
    const opts = this.getOptions()
    const base = this.animating && this.pending != null ? this.pending : this.current
    const raw = base + dir
    if (!opts.loop) {
      if (raw < 0 || raw > this.items.length - 1) {
        if (this.onOverflow) { this.onOverflow(dir > 0 ? 'next' : 'prev') }
        return
      }
    }
    this.syncOptions()
    const target = this.prepareNext(dir, base)
  ...
  destroy() {
    ...
    this.videoTextures.forEach(({ texture, video }) => {
...
const MorphSlider = forwardRef(function MorphSlider(
  {
    items = [],
    startIndex = 0,
    transition = 'melt',
  ...
  useEffect(() => {
    ...
    const engine = new MorphEngine(containerRef.current, {
      ...
      onIndexChange: (i) => {
        setIndex(i)
        if (onIndexChangeRef.current) { onIndexChangeRef.current(i) }
      },
      onOverflow: (dir) => {
        if (onOverflowRef.current) { onOverflowRef.current(dir) }
      }
    })
  ...
  return (
    ...
    >
      <div
        ...
        aria-label="Service video morph slider"
        tabIndex={0}
        onKeyDown={onKeyDown}
      />
      ...
      {showControls && (
        <div className="morph-slider-controls">
          <button type="button" className="morph-slider-btn" aria-label="Previous video" onClick={handlePrev}>
            <svg viewBox="0 0 24 24" width="16" height="16" aria-hidden="true">
              <path d="M15 5l-7 7 7 7" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
          <button type="button" className="morph-slider-btn" aria-label="Next video" onClick={handleNext}>
            <svg viewBox="0 0 24 24" width="16" height="16" aria-hidden="true">
      ...
      {showIndicators && (
        <div className="morph-slider-indicators" role="tablist" aria-label="Videos">
          {items.map((item, i) => (
            <button
              key={i}
              type="button"
